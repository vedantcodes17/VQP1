...
Example 11 Consider two points P and Q with position vectors $$ and $\overrightarrow{\mathrm{O Q}}\quad\vec{a}\quad\vec{b}$ .Findthe positin vctor o a poit R which divies te lie iingP and Q inthe ratio 2:1, (i) internally, and (ii) extrnally.
...
Solution Let $\vec{r}=x\hat{i}+y\hat{j}$ be a unit vector in XY-plane (Eig 10 28) Then fromth b figure, we have $$ and $y=\sin\theta\left(\operatorname{since}|\vec{r}|=1\right)$ b 1. So, we may write the vector $\left[\vec{r}\right.$ as Clearly,
