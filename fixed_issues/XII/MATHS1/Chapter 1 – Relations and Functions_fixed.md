...
(i) transitive, if $\left(a_{1},a_{2}\right)\in\mathbb{R}\mathrm{a n d}\left(a_{2},a_{3}\right)\in\mathbb{R}$ implies that $(a_{1},a_{3})\in\mathbb{R}$ , for all $a_{_1},a_{_2},$
...
R is not transitive. Indeed,$\mathrm{i f}\underline{{\mathrm{L}}}_{1}$ is perpendicular to $\mathrm{L}_{2}$ and $ L_{2}$ is perpendicular to $ L_{3}$ then $\mathrm{L}_{\mathrm{r}}$ can never be perpendicular to

<div style="text-align: center;">Fig 1.1</div>

$ L_{3}$ .In fact,$\mathrm{L}_{\mathrm{r}}$ is parallel to $\mathrm{L}_{3}$ , i.e.,$(\mathrm{L}_{1},\mathrm{L}_{2})\in\mathrm{R},(\mathrm{L}_{2},\mathrm{L}_{3})\in\mathrm{R}\mathrm{b u t}(\mathrm{L}_{1},\mathrm{L}_{3})\notin\mathrm{R}$
...
Slt as $(1,2)\in\mathbb{R}\;\mathrm{but}(2,1)\notin\mathbb{R}$ .Similarly, R is not transitive, as $(1,2)\in\mathbb{R}\;\mathrm{a n d}(2,3)\in\mathbb{R}$ but $(1,3)\notin\mathbb{R}$
...
(i) all elements of $\mathrm{^{i}A}_{i}$ are related to each other, for $\mathrm{a l l}i_{i}$
...
The function $f_{3}$ and $f_{4}$ in Fig 1.2 (iii), (iv) are onto and the function $f_{1}$ in Fig 1.2 (i) is not onto as elements $e,f\mathrm{i n}\mathrm{X}_{_2}$ are not the image of any element in $\mathrm{X}_{\mathrm{_1}}$ under $f_{1}$ .
...
for every Example $x>2$ ,is onto but not one-one.how that the functionf:$\mathbf{N}\rightarrow\mathbf{N}$ ,$$

 不

Solution f is not one-one, as $f(1)=f(2)=1$ - Butf is onto, as given any.

$$ we can choose x as $y\pm1$ such that $f(y+1)=y+1-1=y$ .Also for $1\in\mathbf{N},$ we have $f(1)=1$
...
Definition 8$\mathrm{L e t}f\colon\mathrm{A}\to\mathrm{B}$ and $g:\mathrm{B}\rightarrow\mathrm{C}$ be two functions. Then the composition of f and g, denoted by gof, is defined as thefunction gof:$ A\rightarrow C$ given by
...
$g o f(4)=g(f(4))=g(5)=11\mathrm{a n d}g o f(5)=g(5)=11$
...
Definition 9 A fuctionf:$\mathrm{X\rightarrow Y}$ is defined to be invertible, if there exists a function $g\colon\mathrm{Y}\to\mathrm{X}$ such that $g o f=\mathrm{I}_{\mathrm{x}}\mathrm{a n d}f o g=\mathrm{I}_{\mathrm{y}}$ . The function g is called the inverse of f and is denoted by $if^{-1}$
...
Example $\mathbb{17}\mathrm{L e t}f:\mathbf{N}\to\mathbb{Y}$ be a function defined as $f(x)=4x+3$ , where,
...
Example19 Let R be a relation on the set A of ordered pairs of positive integers defined by $(x,y)\mathbb{R}(u,v)$ if and only $\mathrm{if}xv=vu_{\mathrm{c}}\mathrm{Show}$ that R is an equivalence relation.Solution Clearly,$(x,y)\mathbb{R}(x,y),\forall(x,y)\in\bar{A},$ ,since $x y=y x$ . This shows that R is reflexive. Further,$(x,y)\mathbb{R}(u,v)\Rightarrow xy=yu\Rightarrow uy=vx$ :and hence $(u,v)\mathrel{\operatorname{R}}(x,y)$ . This shows that R is symmetric. Similarly,$(x,y)\operatorname{R}\left(u,v\right)$ and $(u,v)\mathbb{R}(a,b)\Rightarrow xv=yu$ and $ub=via\Rightarrow xv\frac{a}{u}=yu\frac{a}{u}\Rightarrow xv\frac{b}{v}=yu\frac{a}{u}\Rightarrow xb=ya$ and hence $(x,y)\operatorname{\mathbb{R}}\left(a,b\right)$ .Thus, R is transitive. Thus, R is an equivalence relation.
