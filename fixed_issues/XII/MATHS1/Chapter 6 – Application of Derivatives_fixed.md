...
Therefore,$f^{\prime}(x)=0gives\cos3x=0which$ $3x=\frac{\pi}{2},\frac{3\pi}{2}(as x\in\left[0,\frac{\pi}{2}\right])$ implies $3x\in\left[0,\frac{3\pi}{2}\right]\quad\text{S o}x=\frac{\pi}{6}\quad\text{and}\quad\frac{\pi}{2}$ The point$x={\frac{\pi}{6}}$ divides the interval $\left[0,\frac{\pi}{2}\right]$ into two disjoint intervals $\left[0,\frac{\pi}{6}\right]\mathrm{and}\left(\frac{\pi}{6},\frac{\pi}{2}\right)$ 0π6元2
...
<div style="text-align: center;"><html><body><table border="1"><tr><td>Interval</td><td>$$\operatorname{S i g n\;o f}^{'}$f$^{\prime}()$x)$</td><td>Nature of function</td></tr><tr><td>$$\left[0,{\frac{\pi}{4}}\right]$</td><td>$\geq0$</td><td>f is increasing</td></tr><tr><td>

$$\left(\frac{\pi}{4},\frac{5\pi}{4}\right)$</td><td>$\leq0$</td><td>f is decreasing</td></tr><tr><td>$$

\left[\frac{5\pi}{4},2\pi\right]$</td><td>$\geq0$</td><td>f is increasing</td></tr></table></body></html></div>
...
$$[T]0m ; 6.16$\mathrm{S}\equiv\mathrm{S}()$x)=$\mathrm{RP}^{2}+\mathrm{RQ}^{2}=2$x^{2}-40x+1140.$$

$$$\mathrm{S}\equiv\mathrm{S}()$x)=$\mathrm{RP}^{2}+\mathrm{RQ}^{2}=2$x^{2}-40x+1140.$$
...
$$\mathrm{AP}=$x$\mathrm{cm}$ .Note that $\Delta APD\sim\Delta BQC$ .Therefore,$QB=x cm$ . Also, by Pythagoras theorem,$$

\mathrm{DP}=\mathrm{QC}=\sqrt{100-x^{2}}$ . Let A be the area of the trapezium. Then or

$$$\begin{aligned}$A\equiv A(x)&=$\frac{1}{2}($sum of parallel sides(height)\\&=$\frac{1}{2}(2$x+10+10$\left(\sqrt{100-x^2}\right)$\\&=(x+10)$\left(\sqrt{100-x^2}\right)$\\ A'(x)&=(x+10)$\frac{(-2x)}{2\sqrt{100-x^2}}+\left(\sqrt{100-x^2}\right)$\\&=$\frac{-2x^2-10x+100}{\sqrt{100-x^2}}\end{aligned}$ .éd $$

Now

$$$\mathrm{A}^{\prime}($x=0$\text{gives}2$x^{2}+10x-100=0,$\text{i.e.,}$x=5$\text{and}$x=-10.$$
...
$$\left\{$\begin{aligned}\mathrm{S}\end{aligned}$'(x)&=$\frac{2\pi h}{r}()$r-2x)\\ $\mathrm{S}$''(x)&=$\frac{-4\pi h}{r}\right$.$
...
$$$\begin{aligned}$f(x)&=12x$^{\frac{4}{3}}-6$x$^{\frac{1}{3}}$,x\in[-1,1]\\f(x)&=12x$^{\frac{4}{3}}-6$x$^{\frac{1}{3}}$\$\f^{\prime}($x&=16x$^{\frac{1}{3}}-\frac{2}{x^{\frac{2}{3}}}=\frac{2(8x-1)}{x^{\frac{2}{3}}}\end{aligned}$C
$$
...
it follows that $$\sqrt{5}$ is the minimum value of 

$$\sqrt{f(x)}$ . Hence,$$

\sqrt{5}$ is the minimum distance between the soldier and the helicopter.
...
$$$\begin{aligned}(\end{aligned})$i)&f(x)=|x+2|-1\quad&(ii)g(x)=-|x+1|+3\\ (iii)&h(x)=\sin(2x)+5\quad&(iv)f(x)=|\sin4x+3|$()$v)
$$
...
$$$\begin{aligned}\mathrm{V}(\end{aligned})$x)&=x(3-2x)(8-2x)\\&=4x^3-22x^2+24x$\end{aligned}$$
...
Example 37 Manufacturer can sell x items at a price of rupees 

$$\left(5-\frac{x}{100}\right)$ each. The cost price of x items is $$

\mathrm{RS}\left(\frac{x}{5}+500\right)$ . Find the number of items he should sell to earn maximum profit.
...
$$x_{1}<x$_{2}\quad$ in\quad(a,b)\Rightarrow f(x_{1})<f(x)$_{2})\quad$ for all\quad x_{1},x$_{2}\in($a,b).$$

Alternatively, if $f$^{\prime}()$x)\geq0$ for each x in (a, b)

(b) decreasing on (a,b) if

$$x_{1}<x$_{2}\quad\mathrm{in}\quad($a,b\Rightarrow f(x_{1})>f(x)$_{2})\quad\mathrm{for}\quad\mathrm{all}\quad$ x_{1},x$_{2}\in($a,b).$$
...
(i)$$\operatorname{I f}$f$^{\prime}()$x)$ changes sign from positive to negative asincreases through c,i.e.,$$\mathrm{if}$f$^{\prime}()$x)>0$ at every point sufficienlyclose to and to the left of c,and $f$^{\prime}()$x)<0$ at every point sufficientlyclosetoand totherightof c,then c is a point of local maxima.
