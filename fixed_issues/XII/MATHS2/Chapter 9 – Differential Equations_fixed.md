...
\begin{aligned}y&=x\quad&(m&=1,\ c&=0)\\y&=\sqrt{3}x\quad&(m&=\sqrt{3},\ c&=0)\\y&=x+1\quad&(m&=1,\ c&=1)\\y&=-x\quad&(m&=-1,\ c&=0)\\y&=-x-1\quad&(m&=-1,\ c&=-1)\operatorname{etic}.\end{aligned}
...
(b)If the given family $\mathrm{F}_{\bar{2}}$ of curves depends on the parameters a,$b\:(\mathrm{s a y})$ then it is represented by an equation of the from
...
\int x\quad y{\frac{d y}{d x}}=a{\frac{d y}{d x}}\quad{\mathrm{o r}}\quad a={\frac{x\quad y{\frac{d y}{d x}}}{\displaystyle{\frac{d y}{d x}}}}
...
how many years Rs 1000 double itself? ly t e ra $$
...
\begin{aligned}&\mathrm{F}_{1}\left(\lambda x,\lambda y\right)=\lambda^{2}\left(y^{2}+2x y\right)=\lambda^{2}\mathrm{F}_{1}(x,y)\\&\mathrm{F}_{2}\left(\lambda x,\lambda y\right)=\lambda\left(2x-3y\right)=\lambda\mathrm{F}_{2}(x,y)\\ \end{aligned}
...
\mathrm{F}_{1}(x,y)=y^{2}\left(1+\frac{2x}{y}\right)=y^{2}h_{2}\left(\frac{x}{y}\right) ablish 

$$

$$

\mathrm{F}_{2}(x,y)=x^{1}\left(2-\frac{3y}{x}\right)=x^{1}h_{3}\left(\frac{y}{x}\right) ablish 
...
\mathrm{F}_{2}(x,y)=y^{1}\left(2\frac{x}{y}-3\right)=y^{1}h_{4}\left(\frac{x}{y}\right) ablish 

$$

$$

F_{3}(x,y)=x^{0}\cos\left(\frac{y}{x}\right)=x^{0}h_{5}\left(\frac{y}{x}\right) ablish 
...
A differential equation of the form $\frac{dy}{dx}=\mathrm{F}\left(x,y\right)$ is said to be homogenous if $\operatorname{F}(x,y)$ is a homogenous function of degree zero.
...
Differentiating.$y=vx$ with respect to (,)$$
...
1=-1+C,e^{0}\quad\mathrm{o r}\quad C=2
