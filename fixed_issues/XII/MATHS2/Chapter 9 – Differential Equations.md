...
Function φ consists oftwo arbitrary constants (parameters) a, and it is called general oliooftviluio.aunction $\boldsymbol{\phi}_{1}$ contains no arbitrarycbutteiulluoftheam nandhene is called a particular solution of the given differential equation.
...
$$\left(e^{x}+e^{-x}\right)d y-\left(e^{x}-e^{-x}\right)d x=0\quad 使 \quad\frac{dy}{dx}=\left(1+x^{2}\right)\left(1+y^{2}\right)$$
...
$$\mathrm{iv}\quad x^{2}=2y^{2}\log y\quad:\quad(x^{2}+y^{2})\frac{dy}{dx}-x y=0$$
