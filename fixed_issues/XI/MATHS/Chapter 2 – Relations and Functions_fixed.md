...
(i) Polynomial function A function_$f$\colon\mathbb{R}\to\mathbb{R}$ is said to be polynomial function if for each x in $$\mathbf{R}$,y=f(x)=a_{0}+a_{1}x+a_{2}x$^{2}+\ldots+$a_{n}x^{n}$ , where n is a non-negative integer and $a_{0},a_{1},a_{2},...,a$_{n}\in\mathbb{R}$ -F
...
$f$_{\cdot}\mathbb{R}{\rightarrow}\mathbb{R}$ $x$\in\mathbb{R}$ (v)non-neg $f$_{\cdot}\mathbb{R}{\rightarrow}\mathbb{R}$ is defined by ,$f(x)=|x|$ $x,f(x)$ is equal to x.$f(x)=|x|$ hef befmetion for dc 有But for negative values of x, the value of $f(x)$ is the negative of the value of x, i.e,

$$f(x)=\left\{$\begin{aligned}$x,&x\geq0\\ -x,&x<0$\end{aligned}\right$.$

The graph of the modulus function is given in Fig 2.13

(vi) Signum function The function $f$_{:}\mathbf{R}{\rightarrow}\mathbf{R}$ defined by
...
by The function $f(x)=[x],x$\in\mathbf{R}$ ction.$f$_{\cdot}$\;$\mathbb{R}\rightarrow\mathbb{R}$ as assum uch a fu d ,..
...
(iv) Multilication of two ral functions The product (or mulilicatio) oftwo real functions $f$_{:}\mathrm{X}{\rightarrow}\mathbb{R}$ and $g{:}$\mathbb{X}{\to}\mathbb{R}$ is a function $f$_{}{g}{:}\mathrm{{X}{\to}\mathbb{R}}$ defined by $(f g)(x)=f(x)g(x)$ ,for all $x\in X$ O
...
$$\mathrm{X}\to\mathbb{R}$ , where 

$$\vec{X}\subset\mathbb{R}$ $$

\left[\frac{f}{g}\right$. is a function defined by ,ubli
...
$$f$_{\cdot}\mathbf{R}\rightarrow\mathbf{R}\mathrm{by}$f(x)=x+10$$
...
The relation g is defined by $g(x)=\left\{$\begin{aligned}\end{aligned}$x^{2},&0\leq x\leq2\\3x,&2\leq x$\leq10\right$.

Show thatf is a function and g is not a function.

2. 

$$\mathrm{If}$f(x)=x^2$ , find $$
