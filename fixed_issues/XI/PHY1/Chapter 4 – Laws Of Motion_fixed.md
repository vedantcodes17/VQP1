...
A Example 4.9What is the acceleration of the block and trolley system shown in a Fig. 4.12(a), i the cef icient ofkinetic friction between the trolley and the surface is 0.04?What is the tension in the string?(Take $\mathbf{g}=$ 10 m s2).Neglect the mass of the string.
...
Now,$R=\overline{3}m,\overline{g}=9.8m s^{-2},\mu_s=0.1$ . That is,$μ_s$$

Rg=2.94m^2s^{-2}.v=18km/h=5m s^{-1};i.e.$ 一$Rg=2.94m^2s^{-2}.v=18km/h=5m s^{-1};i.e.$ $v^{2}=25\mathrm{m^{2}s^{-2}}$ . The condition is not obeyed.The cyclist will slip while takingthe circular turn.
...
4. 14 Figure 4.16 shows the position-time graph of a particle of mass 4 kg. What is 14$$

 ce (Consider one-dimensional motion only).$t<0,t>4s,0<t<4\mathbf{s?(b)}$ impulse at $t=0$ and
