...
$${\begin{array}{rl}{{\mathrm{I m p u l s e~=~F o r c e~t i m e~d u r a t i o n}}}\\ {{\mathrm{~=~C h a n g e~i n~m o m e n t u m}}}\end{array}}$$
...
$$\begin{array}{rl}{\scriptstyle\mathrm{\itA}_{\mathrm{\it  AISWCT}}\mathrm{\it  \Gamma}}&{\mathrm{Change\;in\;momentum}}\\&{\quad=\;0.15\quad12-(-0.15\;12)}\\&{\quad=\;3.6\;\mathrm{N\;s},}\end{array}$$
...
$$\begin{array}{c}{{\mathbf{F}_{AB}\;=\;-\mathbf{F}_{BA}}}\\ {{\mathrm{(f o r c e~o n}\;A\;\mathrm{by}\;B)\;=\;-\mathrm{(f o r c e~o n}\;B\;\mathrm{by}\;A)}}\end{array}$$
...
However, if you are considering the system of two bodies as a whole,$\mathbf{F}_{AB}$ and $\mathbf{F}_{BA}$ are internal forces of the system $\left(A\not\vdash B\right)$ . They add up to give a null force. Internal forces in a body or a system of particles thus cancel away 
...
$$\left(p_{x}\right)_{initial}=m u\cos30^{\circ},\left(p_{y}\right)_{initial}=-m u\sin30^{\circ}\left(p_{x}\right)_{flnal}=-m u\cos30^{\circ},\left(p_{y}\right)_{flnal}=-m u\sin30^{\circ}$$
...
$$\begin{array}{r}{\pmb{\mathbf{F}}_{AB}\Delta t=\pmb{\mathbf{p}}_{A}^{\prime}-\pmb{\mathbf{p}}_{A}\quad\mathrm{and}\quad}\\ {\left[\pmb{\mathbf{F}}_{BA}\Delta t=\pmb{\mathbf{p}}_{B}^{\prime}-\pmb{\mathbf{p}}_{B}\right.\quad}\end{array}]$$
...
$$\mathbf{F}_{1}+\mathbf{F}_{2}+\mathbf{\bar{F}}_{3}=0\mathbf{F}_{\mathrm{l}}\mathbf{F}_{2}\mathbf{F}_{\mathrm{l}}\mathbf{F}_{2}$$
...
where $F_{\mathrm{1x}},F_{\mathrm{1y}}$ and $F_{1\mathrm{z}}$ are the components of $F_{1}$ along x, y and z directions respectively.
...
$$\begin{aligned}&T_{1}\cos\theta=T_{2}=60N\\&T_{1}\sin\theta=50N\\ \end{aligned}$$
...
$$\begin{array}{rl}&{m a=f_{s}\leq\mu_{s}N=\mu_{s}m g}\\ &{i.e.\quad a\leq\mu_{s}g}\\ &{\therefore\quad a_{max}=\mu_{s}g=0.15\mathrm{\boldmath~x~}10\mathrm{m~s^{-2}~}}\\ &{\quad=1.5\mathrm{\boldmath~m~s^{-2}~}}\end{array}$$
...
$$m g\mathrm{sin}\quad\theta=f_{\mathrm{s}}\quad,\quad m g\quad\mathrm{cos}\quad\theta\quad=N $$

As θ increases, the self-adjusting frictional force $f_{s}$ increases until at $\theta=\theta_{max},\quad f_{s}$ achieves its maximum value,$\left[\left(f_{s}\right)_{\mathrm{max}}=\mu_{s}N_{s}\right.]$ 



Therefore,

$$\tan\theta_{max}=\mu_{s}\quad o r\quad\theta_{max}=\tan^{_{-1}}\mu_{s}$$

When θ becomes just a little more than $\theta_{max}^{-}$ there is a small net force on the block and it begins to slide. Note that $\theta_{max}$ depends only on $\upmu_{s}$ and is independent of the mass of the block.

$$\begin{array}{l}\theta_{max}=15^{\circ},\\ \mu_{s}=\tan15^{\circ},\\ =0.27\end{array}$$
...
$$\begin{array}{r}{T-f_{\mathrm{k}}\;=\;20\;a.}\\ {f_{\mathrm{k}}\quad=\upmu_{\mathrm{k}}N,}\\ {\mu_{\mathrm{k}}\quad=0.04,}\\ {N\;=\;20\;\mathbf{x}\;10,}\\ {=200\;\mathrm{N}.}\end{array}$$
...
$$v_{\mathrm{max}}{=}\sqrt{\mu_{s}R g}\mathrm{Speed}=v $$
...
$$\frac{mg(sin\theta+\mu_s cos\theta)}{cos\theta-\mu_s sin\theta}=\frac{mw_{max}^2}{R}\mathrm{tor}v_{\mathrm{max}}=\left(R g\frac{\mu_{s}+t a n\theta}{1-\mu_{s}t a n\theta}\right)^{\frac{1}{2}}$$
...
Now,$R=\overline{3}m,\overline{g}=9.8m s^{-2},\mu_s=0.1$ . That is,$μ_s$$Rg=2.94m^2s^{-2}.v=18km/h=5m s^{-1};i.e. 一Rg=2.94m^2s^{-2}.v=18km/h=5m s^{-1};i.e. v^{2}=25\mathrm{m^{2}s^{-2}} . The condition is not obeyed.The cyclist will slip while takingthe circular turn.
...
$$\begin{array}{c}{{270-R^{\prime}\;=\;27\quad0.1\mathrm{N}}}\\ {{\mathrm{i e.~}R^{\prime}\;=\;267.3\;\mathrm{N}}}\end{array}\mathrm{s}^{-2}$$
...
$$f_{S}\leq\left(f_{S}\right)_{\mathrm*{max}}=\mu_{S}R f_{\mathbf{k}}=\mu_{k}R $$

$\upmu_{s}$ (co-efficient of static friction)$\upmu_{k}$ fficientofkineticfrictio are constants characteristic of the pair of surfaces in contact.It is found experimentallyth $\upmu_{k}$ than $\upmu_{s}$ 
...
6.Static friction saself-adjutng force up to its limit $\mu_{c}\;N\;(f_{c}\leq\mu_{c}\;N)$ Do not put $f_{\mathrm{s}}=\upmu_{\mathrm{s}}N$ without being sure that themaximum value of static friction is coming inte nlay 
