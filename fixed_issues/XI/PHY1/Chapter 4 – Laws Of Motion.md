...
$$\begin{array}{rl}\scriptstyle\mathrm{\it A}_{\mathrm{\it AISWCT}}\mathrm{\it \Gamma}&\mathrm{Change\;in\;momentum}\\&=\;0.15\quad 12-(-0.15\;12)\\&=\;3.6\;\mathrm{N\;s},\end{array}$$
...
$$\left(p_{x}\right)_{initial}=m u\cos30^{\circ},\left(p_{y}\right)_{initial}=-m u\sin30^{\circ}$$

$$\left(p_{x}\right)_{flnal}=-m u\cos30^{\circ},\left(p_{y}\right)_{flnal}=-m u\sin30^{\circ}$$
...
Since *[Formula could not be recovered from OCR — see source PDF]* by the third law,
...
$$\mathbf{F}_{1}+\mathbf{F}_{2}+\mathbf{\bar{F}}_{3}=0$$
...
$$\begin{array}{rl}&{m a=f_{s}\leq\mu_{s}N=\mu_{s}m g}\\ &{i.e.\quad a\leq\mu_{s}g}\\ &{\therefore\quad a_{max}=\mu_{s}g=0.15\mathrm{~x~}10\mathrm{m~s^{-2}~}}\\ &{\quad=1.5\mathrm{~m~s^{-2}~}}\end{array}$$
...
When θ becomes just a little more than $\theta_{max}^{-}$ there is a small net force on the block and it begins to slide. Note that $\theta_{max}$ depends only on $\mu_{s}$ and is independent of the mass of the block.
...
$$\begin{array}{r}{T-f_{\mathrm{k}}\;=\;20\;a.}\\ {f_{\mathrm{k}}\quad=\mu_{\mathrm{k}}N,}\\ {\mu_{\mathrm{k}}\quad=0.04,}\\ {N\;=\;20\;\mathbf{x}\;10,}\\ {=200\;\mathrm{N}.}\end{array}$$
...
Now,$R=\overline{3}m,\overline{g}=9.8m s^{-2},\mu_s=0.1$ . That is,$μ_sRg=2.94m^2s^{-2}.v=18km/h=5m s^{-1};i.e.$ 一$Rg=2.94m^2s^{-2}.v=18km/h=5m s^{-1};i.e.$ $v^{2}=25\mathrm{m^{2}s^{-2}}$ . The condition is not obeyed.The cyclist will slip while takingthe circular turn.
...
$$f_{S}\leq\left(f_{S}\right)_{\mathrm*{max}}=\mu_{S}R $$

$$f_{\mathbf{k}}=\mu_{k}R $$

$\mu_{s}$ (co-efficient of static friction)$\mu_{k}$ fficientofkineticfrictio are constants characteristic of the pair of surfaces in contact.It is found experimentallyth $\mu_{k}$ than $\mu_{s}$ 
...
6.Static friction saself-adjutng force up to its limit $\mu_{c}\;N\;(f_{c}\leq\mu_{c}\;N)$ Do not put $f_{\mathrm{s}}=\mu_{\mathrm{s}}N$ without being sure that themaximum value of static friction is coming inte nlay 
